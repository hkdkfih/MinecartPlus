...
Classic minecart movement only evaluates one track segment per tick, so a cart traveling one or more blocks per tick can skip a curve. MinecartPlus looks ahead without loading chunks and brakes toward 16 blocks/s before bends. If a cart is spawned or externally accelerated immediately beside a curve, it uses an immediate safety brake instead of leaving the track. Worlds using the improved minecart experiment already iterate track segments and do not need this workaround.
...
2. Copy `MinecartPlus-1.0.2.jar` into `plugins/`.
...
Without a resource pack, all placed tiers look like normal powered rails but behave correctly. See [Geyser setup](docs/GEYSER_SETUP.md) for optional Bedrock item icons and [the Blockbench/resource-pack guide](docs/RESOURCE_PACK_GUIDE.md) for Java item textures and the placed-block limitation.
...
The output is `build/libs/MinecartPlus-1.0.2.jar`.
